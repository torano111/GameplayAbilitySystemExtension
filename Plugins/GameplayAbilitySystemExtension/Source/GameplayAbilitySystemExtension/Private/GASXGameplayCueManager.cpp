...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GASXGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class EGASXEditorLoadMode
{
...
namespace GASXGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("gasx.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via GASXGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UGASXGameplayCueManager::DumpGameplayCues));

	static EGASXEditorLoadMode LoadMode = EGASXEditorLoadMode::LoadUpfront;
}
...
UGASXGameplayCueManager::UGASXGameplayCueManager(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

UGASXGameplayCueManager* UGASXGameplayCueManager::Get()
{
	return Cast<UGASXGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UGASXGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UGASXGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogGASX, Warning, TEXT("UGASXGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UGASXGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (GASXGameplayCueManagerCvars::LoadMode)
	{
	case EGASXEditorLoadMode::LoadUpfront:
		return true;
	case EGASXEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case EGASXEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UGASXGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UGASXGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UGASXGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
	UGASXGameplayCueManager* GCM = Cast<UGASXGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogGASX, Error, TEXT("DumpGameplayCues failed. No UGASXGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogGASX, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogGASX, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogGASX, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogGASX, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogGASX, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogGASX, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogGASX, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogGASX, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogGASX, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogGASX, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogGASX, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogGASX, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UGASXGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UGASXGameplayCueManager* StrongThis = Get())
					{
...
void UGASXGameplayCueManager::HandlePostGarbageCollect()
{
...
void UGASXGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogGASX, Warning, TEXT("UGASXGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UGASXGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (GASXGameplayCueManagerCvars::LoadMode)
	{
	case EGASXEditorLoadMode::LoadUpfront:
...
void UGASXGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject, bool bAlwaysLoadedCue)
{
...
void UGASXGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UGASXGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UGASXGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UGASXGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UGASXGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
