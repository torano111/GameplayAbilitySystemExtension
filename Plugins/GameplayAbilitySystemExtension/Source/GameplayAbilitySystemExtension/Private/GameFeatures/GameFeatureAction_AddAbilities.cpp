...
EDataValidationResult UGameFeatureAction_AddAbilities::IsDataValid(FDataValidationContext& Context) const
{
	...
	for (const FGameFeatureAbilitiesEntry& Entry : AbilitiesList)
	{
		...
		for (const FGASXAbilityGrant& Ability : Entry.GrantedAbilities)
		{
		...
		for (const FGASXAttributeSetGrant& Attributes : Entry.GrantedAttributes)
		{
		...
		for (const TSoftObjectPtr<const UGASXAbilitySet>& AttributeSetPtr : Entry.GrantedAbilitySets)
		{
...
void UGameFeatureAction_AddAbilities::AddActorAbilities(AActor* Actor, const FGameFeatureAbilitiesEntry& AbilitiesEntry, FPerContextData& ActiveData)
{
	...
	if (auto AbilitySystemComponent = FindOrAddComponentForActor<UGASXAbilitySystemComponent>(Actor, AbilitiesEntry, ActiveData))
	{
		...
		for (const FGASXAbilityGrant& Ability : AbilitiesEntry.GrantedAbilities)
		{
			if (!Ability.AbilityType.IsNull())
			{
		...
		for (const FGASXAttributeSetGrant& Attributes : AbilitiesEntry.GrantedAttributes)
		{
			if (!Attributes.AttributeSetType.IsNull())
			{
				...
				if (SetType)
				{
					...
					// Applies InitializeGameplayEffect after the attribute set is set to AbilitySystemComponent
					if (!Attributes.InitializeGameplayEffect.IsNull())
					{
						TSubclassOf<UGameplayEffect> GEClass = Attributes.InitializeGameplayEffect.LoadSynchronous();
						if (GEClass)
						{
							auto GE = GEClass->GetDefaultObject<UGameplayEffect>();
							AbilitySystemComponent->ApplyGameplayEffectToSelf(GE, Level, AbilitySystemComponent->MakeEffectContext());
						}
					}

					if (Attributes.AttributeSetInitializer.IsValid())
					{
						Attributes.AttributeSetInitializer.Apply(AbilitySystemComponent, Level, AbilitySystemComponent->MakeEffectContext());
					}
				}
			}
		}

		for (const TSoftObjectPtr<const UGASXAbilitySet>& SetPtr : AbilitiesEntry.GrantedAbilitySets)
		{
			if (const UGASXAbilitySet* Set = SetPtr.Get())
			{
...
void UGameFeatureAction_AddAbilities::RemoveActorAbilities(AActor* Actor, FPerContextData& ActiveData)
{
	...
	{
		if (auto AbilitySystemComponent = Actor->FindComponentByClass<UGASXAbilitySystemComponent>())
		{
			...
			for (FGASXAbilitySet_GrantedHandles& SetHandle : ActorExtensions->AbilitySetHandles)
			{
